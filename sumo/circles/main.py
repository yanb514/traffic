    ...
    with open(writefilename, 'w') as csv_file:
        ...
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            for vehicle_id in vehicle_ids:
                ...
                distance = 1000-traci.vehicle.getDrivingDistance2D(vehicle_id, 110.97,0.00)#getDistance(vehicle_id)
                # route = traci.vehicle.getRoadID(vehicle_id)
                ...
                # csv_file.write(f"{vehicle_id} {simulation_time} {-1} {distance} {speed} {accel} {-1} {cls} {-1} {-1}\n")
                csv_file.write(f"{vehicle_id} {simulation_time} {-1} {distance} {speed} {accel} {-1} {cls} {-1} {-1}\n") #distance manually adjusted by the circumference of the circle
...
if __name__ == "__main__":
    ...
    # =================== Execute the Python script to draw random veh dist ==================
    ...
    file_name_by_id = sumo_name + "_trajectories_id.csv"
    # write_vehicle_trajectories_to_csv(sumo_name, file_name)
    vis.scatter_time_space("", file_name, highlight_leaders=True)
    # vis.plot_detector_data(xml_file="det.out.xml")

    # =================== reorder data in vehicle ID ==================
    # current_dir = os.getcwd()
    # macro.reorder_by_id(
    #     trajectory_file=os.path.join(current_dir, file_name),
    #     output_file=os.path.join(current_dir, file_name_by_id)
    # )

    # =================== Compute macro properties based on Edie's ==================
    # # compute_macro has to be ordered by vehicleID
    # macro.compute_macro(trajectory_file=os.path.join(current_dir, file_name_by_id), dx=10, dt=10, plot=True)