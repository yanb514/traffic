...
def objective_de(params, param_names, measurement_locations, measured_output):
    ...
    # print(global_iteration_number, driver_param)
    
    # Update SUMO configuration or route files with these parameters
    temp_config_path, temp_path = create_temp_config(driver_param, global_iteration_number)
...
if __name__ == "__main__":
    ...
    # search_space = {key:[min_val[i], max_val[i]] for i, key in enumerate(param_names)}  
    ...
    wrapped_objective = functools.partial(objective_de, param_names=param_names, measurement_locations=measurement_locations, measured_output=measured_output)
    num_workers = 16
    bounds = [(min_val[i], max_val[i]) for i, _ in enumerate(param_names)]
    result = differential_evolution(wrapped_objective, bounds, 
                                    maxiter=1000, popsize=15, workers=lambda f, p: parallel_evaluation(f, p, num_workers))
    print("Optimization result:", result)
    print("Best parameters found:", result.x)
    print("Objective function value at best parameters:", result.fun)
    with open('optuna_studies/result.pkl', 'wb') as f:
